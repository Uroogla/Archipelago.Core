...
namespace Archipelago.Core.Util.PlatformMemory
{
    public class LinuxMemory : IMemory
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (bytesRead.ToInt64() > 0)
                    {
                        ...
                        string tmp = "chars: ";
                        foreach (var b in lpBuffer)
                        {
                            if (b != 0) tmp += b;
                        }
                        Log.Logger.Warning($"{bytesRead} {tmp}");
                        return true;
                    }
        ...
        {
            //return 0x73d2dc31e000;
            try
            {
                Log.Logger.Warning($"{processHandle} {moduleBase} {exportName}");
                // Read ELF header
                ...
                Log.Logger.Warning($"{processHandle} {moduleBase} {shoff}");
                ...
                Log.Logger.Warning($"{shentsize * shnum} {sectionHeaders.Length} {bytesRead}");
                string tmp = "Chars: ";
                foreach (var b in sectionHeaders)
                {
                    if ((byte)b != 0)
                    {
                        tmp += (byte)b;
                ...
                {
                    Log.Logger.Warning($"{dynstrOffset} {dynsymOffset}");
